...
const toggleDropdown = (id, setButtonState) => {
  const element = document.getElementById(id);

  if (element.classList.contains("hidden")) {
    element.classList.remove("hidden");
    setButtonState(false); // hide Read More
  } else {
    element.classList.add("hidden");
    setButtonState(true); // show Read More again
  }
};

function Academic() {
  const [showCoursesButton, setShowCoursesButton] = useState(true);
  const [showResearchButton, setShowResearchButton] = useState(true);

  return (
    <section
      id="academic"
      className="scroll-mt-24 bg-black text-white pt-10 pb-20 px-8 md:px-12 lg:px-24 border-b border-gray-700"
    >
        ...
        <div className="bg-neutral-900 border border-gray-700 p-6 rounded-lg hover:border-green-500 transition">
          ...
          <div>
            {showCoursesButton && (
              <button
                onClick={() =>
                  toggleDropdown("coursesDropdown", setShowCoursesButton)
                }
            ...
            >
              ...
              <button
                ...
                className="mt-4 text-green-400 hover:text-green-500 transition"
              >
        ...
        <div className="bg-neutral-900 border border-gray-700 p-6 rounded-lg hover:border-green-500 transition">
          ...
          <div>
            {showResearchButton && (
              <button
                onClick={() =>
                  toggleDropdown("researchDropdown", setShowResearchButton)
                }
                className="inline-flex items-center mt-4 text-green-400 hover:text-green-500 transition"
              >
                Read More <span className="ml-1">→</span>
